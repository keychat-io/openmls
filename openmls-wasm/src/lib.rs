...
// #[wasm_bindgen]
// impl Group {
//     pub fn create_new(provider: &Provider, founder: &Identity, group_id:
// &str) -> Group {         let group_id_bytes =
// group_id.bytes().collect::<Vec<_>>();
...
//         let welcome = match MlsMessageIn::tls_deserialize(&mut
// welcome)?.extract() {             MlsMessageBodyIn::Welcome(welcome) =>
// Ok(welcome),             other =>
// Err(openmls::error::ErrorString::from(format!(                 "expected a
// message of type welcome, got {other:?}",             ))),
//         }?;
//         let config = MlsGroupJoinConfig::builder().build();
//         let mls_group =
//             StagedWelcome::new_from_welcome(&provider.0, &config, welcome,
// Some(ratchet_tree.0))?                 .into_group(&provider.0)?;
...
//             self.mls_group
//                 .propose_add_member(provider.as_ref(), &sender.keypair,
// &new_member.0)?;
...
//     pub fn merge_pending_commit(&mut self, provider: &mut Provider) ->
// Result<(), JsError> {         self.mls_group
...
//         let (commit_msg, welcome_msg, _group_info) = self
//             .mls_group
//             .commit_to_pending_proposals(provider.as_ref(),
// &sender.keypair)?;
...
//     fn native_join(provider: &Provider, mut welcome: &[u8], ratchet_tree:
// RatchetTree) -> Group {         let welcome =
// MlsMessageIn::tls_deserialize(&mut welcome)             .unwrap()
//             .into_welcome()
...
//         let mut chess_club_alice = Group::create_new(&alice_provider, &alice,
// "chess club");

//         let bob_key_pkg = bob.key_package(&bob_provider);

//         let add_msgs = chess_club_alice
//             .native_propose_and_commit_add(&alice_provider, &alice,
// &bob_key_pkg)             .map_err(js_error_to_string)
//             .unwrap();
...
//         let chess_club_bob = Group::native_join(&bob_provider,
// &add_msgs.welcome, ratchet_tree);
