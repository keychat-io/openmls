...
//         let name = format_ident!("{}_rustcrypto_{}", fn_name,
// ciphersuite_name);         let test_fun = quote! {
//             #(#attrs)*
...
// crypto::OpenMlsCrypto, storage::StorageProvider as StorageProviderTrait};
//                 use openmls_traits::OpenMlsProvider;

//                 type Provider = OpenMlsRustCrypto;
//                 type StorageProvider = <Provider as
// openmls_traits::OpenMlsProvider>::StorageProvider;                 type
// StorageError = <StorageProvider as
// StorageProviderTrait<{openmls_traits::storage::CURRENT_VERSION}>>::Error;
...
//             let val = ciphersuite as u16;
//             let ciphersuite_name = format!("{:?}", ciphersuite);
//             let name = format_ident!("{}_sqlite_{}", fn_name,
// ciphersuite_name);             let test_fun = quote! {
//                 #(#attrs)*
//                 #[allow(non_snake_case)]
//                 #[test]
...
// Codec, Connection};                     use openmls_traits::OpenMlsProvider;
//                     use openmls_traits::{types::Ciphersuite,
// crypto::OpenMlsCrypto, storage::StorageProvider as StorageProviderTrait};
...
//                         fn to_vec<T: serde::Serialize>(value: &T) ->
// Result<Vec<u8>, Self::Error> {
// serde_json::to_vec(value)                         }

//                         fn from_slice<T: serde::de::DeserializeOwned>(slice:
// &[u8]) -> Result<T, Self::Error> {
// serde_json::from_slice(slice)                         }
//                     }

//                     struct OpenMlsSqliteTestProvider {
//                         crypto: RustCrypto,
//                         storage: SqliteStorageProvider<JsonCodec,
// Connection>,                     }

//                     impl Default for OpenMlsSqliteTestProvider {
//                         fn default() -> Self {
//                             let connection =
// Connection::open_in_memory().unwrap();                             let mut
// storage = SqliteStorageProvider::new(connection);
// storage.initialize().unwrap();                             Self {
...
// SqliteStorageProvider<JsonCodec, Connection>;
...
//             let val = ciphersuite as u16;
//             let ciphersuite_name = format!("{:?}", ciphersuite);
//             let name = format_ident!("{}_libcrux_{}", fn_name,
// ciphersuite_name);             let test_fun = quote! {
//                 #(#attrs)*
//                 #[allow(non_snake_case)]
//                 #[test]
...
// openmls_traits::OpenMlsProvider>::StorageProvider;                     type
// StorageError = <StorageProvider as
// openmls_traits::storage::StorageProvider<{openmls_traits::storage::CURRENT_VERSION}>>::Error;
//
...
//                     // When cross-compiling the supported ciphersuites may be
// wrong.                     // They are set at compile-time.
//                     if provider.crypto().supports(ciphersuite).is_err() {
//                         eprintln!("Skipping unsupported ciphersuite
// {ciphersuite:?}.");                         return;
//                     }
