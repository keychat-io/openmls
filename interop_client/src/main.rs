...
// use openmls::{
//     credentials::{BasicCredential, Credential, CredentialType,
// CredentialWithKey},     framing::{MlsMessageBodyIn, MlsMessageIn,
// MlsMessageOut, ProcessedMessageContent},     group::{
//         GroupEpoch, GroupId, MlsGroup, MlsGroupCreateConfig,
// MlsGroupJoinConfig, StagedWelcome,         WireFormatPolicy,
...
// OpenMlsProvider}; use tls_codec::{Deserialize, Serialize};
...
// /// This struct contains the state for a single MLS client. The interop
// client /// doesn't consider scenarios where a credential is re-used across
// groups, so /// this simple structure is sufficient.
...
// /// This is the main state struct of the interop client. It keeps track of
// the /// individual MLS clients, as well as pending key packages that it was
// told to /// create. It also contains a transaction id map, that maps the
// `u32` /// transaction ids to key package hashes.
// pub struct MlsClientImpl {
//     groups: Mutex<Vec<InteropGroup>>,
//     pending_state: Mutex<HashMap<Vec<u8>, PendingState>>,
//     transaction_id_map: Mutex<HashMap<u32, Vec<u8>>>, // Indirection, linking
// to pending key packages }
...
// // A helper function translating the bool in the protobuf to OpenMLS'
// WireFormat pub fn wire_format_policy(encrypt: bool) -> WireFormatPolicy {
//     match encrypt {
...
//     if !bytes.is_empty() {
//         let ratchet_tree =
// RatchetTreeIn::tls_deserialize_exact(bytes.as_slice()).unwrap();
...
//     #[instrument(skip_all)]
//     async fn name(&self, request: Request<NameRequest>) ->
// Result<Response<NameResponse>, Status> {         let request =
// request.get_ref();         info!(?request, "Request");
...
//         let ciphersuite = Ciphersuite::try_from(request.cipher_suite as
// u16).unwrap();         let credential =
// BasicCredential::new(request.identity.clone());         let signature_keys =
// SignatureKeyPair::new(ciphersuite.signature_algorithm()).unwrap();
//         signature_keys.store(provider.storage()).unwrap();

//         let wire_format_policy =
// wire_format_policy(request.encrypt_handshake);         // Note: We just use
// some values here that make live testing work.         //       There is
// nothing special about the used numbers and they         //       can be
// increased (or decreased) depending on the available scenarios.         let
// mls_group_config = MlsGroupCreateConfig::builder()
// .ciphersuite(ciphersuite)             .max_past_epochs(32)
...
//         let transaction_id: [u8; 4] =
// crypto_provider.rand().random_array().unwrap();         let transaction_id =
...
//         trace!(identity =
// String::from_utf8_lossy(&request.identity).to_string());
...
// nothing special about the used numbers and they         //       can be
// increased (or decreased) depending on the available scenarios.         let
// mls_group_config = MlsGroupJoinConfig::builder()
// .max_past_epochs(32)             .number_of_resumption_psks(32)
//
// .sender_ratchet_configuration(SenderRatchetConfiguration::default())
//             .use_ratchet_tree_extension(true)
//             .wire_format_policy(wire_format_policy)
...
//         let (my_key_package, _my_credential, my_signature_keys,
// crypto_provider) =             pending_key_packages
...
//         // Store the key package in the key store with the hash reference as
// id         // for retrieval when parsing welcome messages.
//         crypto_provider
//             .storage()
//             .write_key_package(
//                 &my_key_package
...
//         let welcome = MlsMessageIn::tls_deserialize(&mut
// request.welcome.as_slice())             .map_err(|_| Status::aborted("failed
// to deserialize MlsMessage with a Welcome"))?             .into_welcome()
//             .expect("expected a welcome");

//         let ratchet_tree =
// ratchet_tree_from_config(request.ratchet_tree.clone());
...
//                 let msg =
//                     MlsMessageIn::tls_deserialize(&mut
// request.group_info.as_slice()).unwrap();

//                 match msg.extract() {
//                     MlsMessageBodyIn::GroupInfo(verifiable_group_info) =>
// verifiable_group_info,                     other => panic!("Expected
// `MlsMessageBodyIn::GroupInfo`, got {other:?}."),                 }
//             };
...
//                 let credential =
// BasicCredential::new(request.identity.to_vec());
...
//                 let wire_format_policy =
// wire_format_policy(request.encrypt_handshake);
...
//             let (mut group, commit, _group_info) =
// MlsGroup::join_by_external_commit(                 &provider,
//                 &signer,
//                 ratchet_tree,
...
//             debug!(commit=?group.pending_commit(), "Merging pending
// commit.");             group.merge_pending_commit(&provider).unwrap();

//             (
//                 InteropGroup {
//                     wire_format_policy:
// mls_group_config.wire_format_policy(),                     group,
...
//             .ok_or_else(|| Status::new(Code::InvalidArgument, "unknown
// state_id"))?;         trace!("   in epoch {:?}",
// interop_group.group.epoch());
...
//         let message = MlsMessageIn::tls_deserialize(&mut
// request.ciphertext.as_slice())             .map_err(|_|
// Status::aborted("failed to deserialize ciphertext"))?;         debug!("
// Deserialized `MlsMessageIn`.");         trace!(?message);
...
// unreachable!(),             ProcessedMessageContent::StagedCommitMessage(_)
// => unreachable!(),         };
...
//             secret: &[u8],
//         ) -> Result<(), Status> {
//             let psk_id = PreSharedKeyId::new(ciphersuite,
// crypto_provider.rand(), external_psk)                 .map_err(|_|
// Status::internal("unable to create PreSharedKeyId from raw psk_id"))?;
//             psk_id
//                 .store(crypto_provider, secret)
//                 .map_err(|_| Status::new(Code::Internal, "unable to store
// PSK"))?;             Ok(())
//         }

//         // This might be for a transaction ID or a state ID, so either a
// group, or not.         // Transaction IDs are random. We assume that if it
// exists, it is what we want.         let transaction_id_map =
// self.transaction_id_map.lock().unwrap();         let pending_state_id =
// transaction_id_map.get(&request.state_or_transaction_id);         if let
// Some(pending_state_id) = pending_state_id {             let mut pending_state
// = self.pending_state.lock().unwrap();             let pending_state =
// pending_state                 .get_mut(pending_state_id)
//                 .ok_or(Status::internal("Unable to retrieve pending
// state"))?;
...
//         let key_package = MlsMessageIn::tls_deserialize(&mut
// request.key_package.as_slice())             .map_err(|_|
...
//         //       can be increased (or decreased) depending on the available
// scenarios.         let mls_group_config = MlsGroupJoinConfig::builder()
//             .use_ratchet_tree_extension(true)
//             .max_past_epochs(32)
//             .number_of_resumption_psks(32)
...
//         interop_group
//             .group
//             .set_configuration(interop_group.crypto_provider.storage(),
// &mls_group_config)             .map_err(|err| {
//                 tonic::Status::internal(format!("error setting configuration:
// {err}"))             })?;
...
// BasicCredential::new(request.removed_id.clone());         trace!("   for
// credential: {removed_credential:x?}");
...
//             .get_mut(request.state_id as usize)
//             .ok_or_else(|| Status::new(Code::InvalidArgument, "unknown
// state_id"))?;         let group = &mut interop_group.group;

//         trace!(epoch=?group.epoch(), "Current group state.");

//         // Proposals by reference. These proposals are standalone proposals.
// They should         // be appended to the proposal store.
//         for proposal in &request.by_reference {
//             trace!("Handling proposal by reference.");

//             let message = MlsMessageIn::tls_deserialize(&mut
// proposal.as_slice())                 .map_err(|_| Status::aborted("failed to
// deserialize proposal"))?;             if
// interop_group.messages_out.contains(&message) {
// trace!("Skipping processing of own proposal");                 continue;
//             }
...
//                         .map_err(|err| {
//                             tonic::Status::internal(format!("error storing
// proposal: {err}"))                         })?;
//                 }
//                 ProcessedMessageContent::ExternalJoinProposalMessage(_) =>
// unreachable!(),
...
//         // Proposals by value. These proposals are inline proposals. They
// should be         // converted into group operations.
//         for proposal in &request.by_value {
//             let proposal_type =
// String::from_utf8_lossy(&proposal.proposal_type).to_string();
...
// let key_package = key_package                         .into_keypackage()
//                         .ok_or(Status::invalid_argument("Message was not a
// key package"))?;
...
//                             key_package,
//                         )
//                         .map_err(|_| Status::internal("Unable to generate
// proposal by value"))?                 }
//                 "remove" => {
//                     let removed_credential =
// BasicCredential::new(proposal.removed_id.clone());
...
// )                     .map_err(|_| Status::internal("Unsupported proposal
// type (resumption PSK)"))?;
...
//                         "Unsupported proposal type (group context
// extension)",                     ))
//                 }
//                 _ => return Err(Status::invalid_argument("Invalid proposal
// type")),             };
...
//         debug!(commit=?group.pending_commit(), "Pending commit created.
// (Note: Not merged yet.)");
...
//                 .export_ratchet_tree()
//                 .tls_serialize_detached()
//                 .map_err(|_| Status::aborted("failed to serialize ratchet
// tree"))?         } else {
//             vec![]
//         };
...
// proposal.as_slice())                 .map_err(|_| Status::aborted("failed to
// deserialize proposal"))?;             if
// interop_group.messages_out.contains(&message) {                 trace!("
// skipping processing of own proposal");                 continue;
//             }
//             trace!("   processing proposal ...");
//             let processed_message = group
//                 .process_message(
...
//         let message =
//             MlsMessageIn::tls_deserialize(&mut
// request.commit.as_slice()).map_err(|_| {                 error!("Failed to
// deserialize ciphertext");                 Status::aborted("failed to
...
//         let epoch_authenticator =
// group.epoch_authenticator().as_slice().to_vec();         let response =
// HandleCommitResponse {             state_id: request.state_id,
//             epoch_authenticator,
//         };
...
//                 .export_ratchet_tree()
//                 .tls_serialize_detached()
//                 .map_err(|_| Status::internal("Unable to serialize ratchet
// tree."))?         } else {
//             vec![]
//         };
...
//             external_psk,
//         )
//         .map_err(|_| Status::internal("unable to create PreSharedKeyId from
// raw psk_id"))?;
...
//                 psk_id,
//             )
//             .map_err(|_| Status::internal("failed to generate psk
// proposal"))?;
...
//     async fn free(&self, _request: Request<FreeRequest>) ->
// Result<Response<FreeResponse>, Status> {         debug!("Got Free request");
