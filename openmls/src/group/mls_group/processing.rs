...
impl MlsGroup {
    ...
    pub fn process_message<Provider: OpenMlsProvider>(
        ...
        let ratchet_key  = sender_ratchet.and_then(|sr| sr.get_decryption_ratchet_secret()).map(|rs| rs.ratchet_head.secret.as_slice().to_vec());
        // if ratchet_key.is_some() {
        //     println!("The message_secrets is {:?}", ratchet_key.unwrap());
        // }
        ...
        Ok((self.process_unverified_message(
            provider,
            unverified_message,
            old_epoch_keypairs,
            leaf_node_keypairs,
