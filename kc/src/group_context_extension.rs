...
//! Copyied from White Noise
//! https://github.com/erskingardner/nostr-openmls/blob/master/src/nostr_group_data_extension.rs
...
impl NostrGroupDataExtension {
    ...
    /// * `admin_identities` - A list of Nostr public keys that have admin
    ///   privileges
    /// * `relays` - A list of relay URLs where group messages will be published
    ///
    /// # Returns
    ///
    /// A new NostrGroupDataExtension instance with a randomly generated group
    /// ID and the provided parameters converted to bytes. This group ID
    /// value is what's used when publishing events to Nostr relays for the
    /// group.
    pub fn new(
        name: String,
        description: String,
    ...
    /// # Arguments
    ///
    /// * `group_context` - Reference to the GroupContext containing the
    ///   extension
    ///
    /// # Returns
    ///
    /// * `Ok(NostrGroupDataExtension)` - Successfully extracted and
    ///   deserialized extension
    /// * `Err(NostrGroupDataExtensionError)` - Failed to find or deserialize
    ///   the extension
...
mod tests {
    ...
    // setup with mocked MlsGroup and GroupContext objects to test properly
}
